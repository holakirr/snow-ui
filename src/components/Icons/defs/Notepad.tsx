import type { CustomIconWeights } from "..";

export const NotepadWeights: CustomIconWeights = new Map([
	[
		"regular",
		<>
			<path d="M6 5C6 3.89543 6.89543 3 8 3H20L26 9V27C26 28.1046 25.1046 29 24 29H8C6.89543 29 6 28.1046 6 27V5Z" />
			<path d="M26 9L20 3V7C20 8.10457 20.8954 9 22 9H26Z" />
			<path
				fillRule="evenodd"
				clipRule="evenodd"
				d="M9.5 13C9.22386 13 9 13.2239 9 13.5C9 13.7761 9.22386 14 9.5 14H22.5C22.7761 14 23 13.7761 23 13.5C23 13.2239 22.7761 13 22.5 13H9.5ZM9.5 16C9.22386 16 9 16.2239 9 16.5C9 16.7761 9.22386 17 9.5 17H22.5C22.7761 17 23 16.7761 23 16.5C23 16.2239 22.7761 16 22.5 16H9.5ZM9 19.5C9 19.2239 9.22386 19 9.5 19H22.5C22.7761 19 23 19.2239 23 19.5C23 19.7761 22.7761 20 22.5 20H9.5C9.22386 20 9 19.7761 9 19.5ZM9.5 22C9.22386 22 9 22.2239 9 22.5C9 22.7761 9.22386 23 9.5 23H22.5C22.7761 23 23 22.7761 23 22.5C23 22.2239 22.7761 22 22.5 22H9.5Z"
			/>
			<path d="M24.8493 12.7071C25.2398 12.3166 25.873 12.3166 26.2635 12.7071L29.0919 15.5355C29.4825 15.9261 29.4825 16.5592 29.0919 16.9497L16.364 29.6777L12.1214 25.435L24.8493 12.7071Z" />
			<path d="M12.1213 29.6777V25.4351L16.364 29.6777H12.1213Z" />
			<path d="M12.1213 29.6776V28.2634L13.5356 29.6776H12.1213Z" />
			<rect
				x="23.4351"
				y="14.1213"
				width="6"
				height="3"
				transform="rotate(45 23.4351 14.1213)"
			/>
			<defs>
				<linearGradient
					id="paint0_linear_52206_162198"
					x1="16"
					y1="3"
					x2="16"
					y2="31"
					gradientUnits="userSpaceOnUse"
				>
					<stop stopColor="#F1ECF7" />
					<stop offset="1" stopColor="#E8DEF2" />
				</linearGradient>
				<linearGradient
					id="paint1_linear_52206_162198"
					x1="27.6777"
					y1="14.1213"
					x2="14.2427"
					y2="27.5563"
					gradientUnits="userSpaceOnUse"
				>
					<stop stopColor="#FF6659" />
					<stop offset="1" stopColor="#FF8749" />
				</linearGradient>
				<linearGradient
					id="paint2_linear_52206_162198"
					x1="14.2427"
					y1="27.5564"
					x2="12.1213"
					y2="29.6777"
					gradientUnits="userSpaceOnUse"
				>
					<stop stopColor="#FFBA9F" />
					<stop offset="1" stopColor="#DE8893" />
				</linearGradient>
				<linearGradient
					id="paint3_linear_52206_162198"
					x1="23.7487"
					y1="15.0503"
					x2="30.8198"
					y2="15.0503"
					gradientUnits="userSpaceOnUse"
				>
					<stop stopColor="#E3DBEF" />
					<stop offset="0.807292" stopColor="#F6F1FE" />
					<stop offset="1" stopColor="#EADEFC" />
				</linearGradient>
			</defs>
		</>,
	],
]);
